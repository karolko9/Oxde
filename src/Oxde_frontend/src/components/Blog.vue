...
<script>
import { Oxde_backend } from 'declarations/Oxde_backend/index';
...
export default {
    ...
    methods: {
        async dodajWpisy() {
            await Oxde_backend.dodaj_wpis(this.nowyBlog);
            await this.pobierzWpisy();
        },
        async deleteWpis(index) {
            await Oxde_backend.usun_wpis(index);
            await this.pobierzWpisy();
        },
        async pobierzWpisy() {
            this.wpisy = await Oxde_backend.odczytaj_wpisy();
        }
    },
    async mounted(){
        this.pobierzWpisy()
